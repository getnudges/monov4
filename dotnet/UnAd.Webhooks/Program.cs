...
using UnAd.Webhooks;
using UnAd.Webhooks.Endpoints;
using UnAd.Webhooks.Endpoints.Handlers;
using UnAd.Webhooks.Stripe;
using UnAd.Webhooks.Stripe.Commands;
...
builder.Services.AddHttpClient<IUnAdClient>(UnAdClient.ClientName)
...
builder.Services.AddSingleton(s => new StripeEventCommandProcessorBuilder()
    ...
    .AddHandler("price.deleted", new PriceDeletedCommand(
        s.GetRequiredService<IUnAdClient>()))
    ...
    .AddHandler("checkout.session.completed", new CheckoutSessionCompletedCommand(
        s.GetRequiredService<IUnAdClient>(), s.GetRequiredService<IStripeClient>()))
    .Build());
...
builder.Services.AddUnAdClient()
