...
using UnAd.Webhooks.Twilio.Commands;

namespace UnAd.Webhooks.Endpoints.Handlers;

internal sealed partial class AnnouncementCommand(IUnAdClient unAdClient,
    ...
    public async Task<Result<MessagingResponse, Exception>> InvokeAsync(TwilioEventContext context, CancellationToken cancellationToken) =>
        await unAdClient.GetClientByPhoneNumber(context.From, cancellationToken).Map<IGetClientByPhoneNumber_ClientByPhoneNumber, MessagingResponse, Exception>(async client => {
...
internal sealed partial class AnnouncementConfirmCommand(IUnAdClient unAdClient,
                                                         ICacheClient<string> cache,
    ...
    public async Task<Result<MessagingResponse, Exception>> InvokeAsync(TwilioEventContext context, CancellationToken cancellationToken) =>
        await unAdClient.GetClientByPhoneNumber(context.From, cancellationToken).Map(async client => {
            if (client.Subscription?.Status != "ACTIVE") {
                return new NotCustomerException();
            }
    ...
    private async Task<Result<int, Exception>> SendMessages(string clientPhone, Dictionary<string, string> replacements, CancellationToken cancellationToken) =>
        await unAdClient.GetClientByPhoneNumber(clientPhone, cancellationToken).Map<IGetClientByPhoneNumber_ClientByPhoneNumber, int, Exception>(async client => {
